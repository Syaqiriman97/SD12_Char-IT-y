<?php
  ...
  {
    if(@$_GET['daddress'] && $_SESSION['key']=='admin') 
    {
      $daddress=@$_GET['daddress'];
      $r1 = mysqli_query($con,"DELETE FROM personinneed WHERE address='$daddress' ") or die('Error');
      header("location:dashboard.php?q=2");
    }
    if(@$_GET['dphonenumber'] && $_SESSION['key']=='admin') 
    {
      $dphonenumber=@$_GET['dphonenumber'];
      $r2 = mysqli_query($con,"DELETE FROM donater WHERE phonenumber='$dphonenumber' ") or die('Error');
    ...
    {
      $dname=@$_GET['dname'];
      $result = mysqli_query($con,"DELETE FROM donationstatusform WHERE name='$dname' ") or die('Error');
